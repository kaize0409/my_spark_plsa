...
class Document(val docName: String, val tokens: SparseVector[Int], val contents : Seq[String]) extends Serializable {
    /**
     *
     * @return number of different tokens in the collection
     */
 ...
 */
class TokenEnumeration private[topicmodeling](val alphabet: Index[String]) extends Serializable {
    ...
    /*def transform(docName: String, rawDocument: Seq[String]): Document = {
        val wordsMap = rawDocument.map(alphabet.apply)
          .filter(_ != -1)
          .foldLeft(Map[Int, Int]().withDefaultValue(0))((map, word) => map + (word -> (1 + map(word))))
    ...
    def transform(rawDocument: (String, Seq[String])): Document = {
        ...
        val tokens = new SparseVector[Int](words, words.map(word => wordsMap(word)), alphabet.size)//返回词的index和出现次数
        // new Document(tokens)
        new Document(rawDocument._1, tokens, rawDocument._2)
    }
}
