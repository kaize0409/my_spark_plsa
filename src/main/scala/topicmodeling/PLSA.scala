...
class PLSA(@transient protected val sc: SparkContext,
    ...
    override def foldIn(documents: RDD[Document],
        ...
        EM(documents, sc.broadcast(globalParams.phi), globalParams.alphabetSize, foldingIn = true)._1//第一个返回值
    }
    ...
    private def EM(documents: RDD[Document],
        ...
        val alphabetSize = getAlphabetSize(documents)
        logInfo(s"alphabet size : $alphabetSize")
...
//            val perplexity_value = perplexity(topicsBC, parameters, collectionLength)
//            println("Perplexity=" + perplexity_value)
//        }
        //val parameters_value = parameters.collect()
        if (numberOfIteration == numberOfIterations) {
            (parameters, topicsBC)
        } else {
            val newParameters = parameters.map(param => param.getNewTheta(topicsBC.value)).cache()//获得新的p(z | d)
            val globalCounters = getGlobalCounters(parameters, topicsBC, alphabetSize)//获得新的p(w | z)
            ...
            //*************
            //val newParameters_value = newParameters.collect()
            logInfo("Iteration number " + numberOfIteration)
            logInfo("Perplexity=" + perplexity(newTopics, newParameters, collectionLength))
            //*************
    ...
    private def getGlobalCounters(parameters: RDD[DocumentParameters],
      ...
      //agggregate首先将每个partition中的每个值依次与输入参数迭代操作，最后由combine函数将各partition结果汇总
    }
